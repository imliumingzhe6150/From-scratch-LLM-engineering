...
def run_linear(
    ...
    """
    # Match the reference weight's device and dtype when constructing the
    # module. This also prevents an unnecessary CPU/device conversion.
    linear = Linear(
        in_features=d_in,
        out_features=d_out,
    ...
    # The test supplies a known parameter tensor. Loading it into our module
    # ensures the test evaluates the forward computation rather than the
    # random initialization performed by Linear.__init__.
...
def run_embedding(
    ...
    # Replace the random initialization with the reference embedding table so
    # the test isolates and verifies our lookup behavior.
...
def run_swiglu(
    ...
    # Use the supplied reference parameters so the snapshot test isolates the
    # SwiGLU data flow: SiLU(W1 x) * (W3 x), followed by W2.
...
def run_rmsnorm(
    ...
    # Load the reference gain vector so the snapshot test checks our RMS
    # calculation and broadcasting rather than the default all-ones gain.
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe(input_path = input_path, special_tokens = special_tokens, vocab_size = vocab_size, **kwargs)
